...
async def add_jobtype(callback: types.CallbackQuery, tg_id: str, state: FSMContext, session: Any) -> None:
    ...
    # FOR EDIT
        ...
        await state.set_state(EditJobetype.input_category)
        all_categories: list[Category] = await AsyncOrm.get_all_categories(session)
        text = await t.t("select_category", lang)
        keyboard = await kb.select_category(all_categories, lang)
        await callback.message.edit_text(text, reply_markup=keyboard.as_markup())


# ONLY FOR EDIT
@router.callback_query(EditJobetype.input_category)
@router.callback_query(F.data.split("|")[0] == "back-from-input-new-title", EditJobetype.input_new_jobtype_title)
async def get_category_for_jobtypes(callback: types.CallbackQuery, tg_id, state: FSMContext, session: Any) -> None:
    """Для изменения получаем категорию в которой мы будем менять"""
    lang = r.get(f"lang:{tg_id}").decode()
    ...
    jobtypes: list[Jobtype] = await AsyncOrm.get_job_types_by_category(category.id, session)
    await state.update_data(category_id_to_edit=category.id)

    text = f"\"{category.emoji + ' ' if category.emoji else ''} {await t.t(category.title, lang)}\"\n"
    text += f"{await t.t('select_jobtype', lang)}"
    keyboard = await kb.jobetypes_keyboard(jobtypes, lang)
...
@router.callback_query(F.data.split("|")[0] == "back-from-translate_1", EditJobetype.translate_1)  # for Back button
async def get_jobtype_to_edit(callback: types.CallbackQuery, tg_id, state: FSMContext, session: Any) -> None:
    """Получаем jobtype и предлагаем ввести новый jobtype"""
    lang = r.get(f"lang:{tg_id}").decode()
    ...
    keyboard = await kb.back_keyboard(lang, f"back-from-input-new-title|{data['category_id_to_edit']}")

    prev_message = await callback.message.edit_text(text, reply_markup=keyboard.as_markup())
    await state.update_data(prev_message=prev_message)


@router.message(EditJobetype.input_new_jobtype_title)
@router.callback_query(F.data == "back-from-translate_2", EditJobetype.translate_2)
async def get_new_jobtype_title(message: types.Message | types.CallbackQuery, tg_id: str, state: FSMContext) -> None:
    """Принимаем новое название для группы узлов"""
    lang = r.get(f"lang:{tg_id}").decode()
    data = await state.get_data()
    ...
    keyboard = await kb.back_keyboard(lang, f"back-from-translate_1|{data['jobtype_id']}")
...
async def get_category(message: types.Message | types.CallbackQuery, tg_id, state: FSMContext, session: Any) -> None:
    """Получение типа"""
    lang = r.get(f"lang:{tg_id}").decode()
    data = await state.get_data()
...
@router.callback_query(F.data.split("|")[0] == "select_categories_done", EditJobetype.input_jobtype)
...
async def get_translate_1(callback: types.CallbackQuery, tg_id: str, state: FSMContext) -> None:
    ...
    # FOR EDIT
    elif current_state in (EditJobetype.input_jobtype, EditJobetype.translate_2):
        if current_state == EditJobetype.input_jobtype:
            jobtype_id = int(callback.data.split("|")[1])
            await state.update_data(jobtype_id=jobtype_id)
        await state.set_state(EditJobetype.translate_1)
...
async def confirm_create_jobtype(callback: types.CallbackQuery, tg_id: str, state: FSMContext, session: Any) -> None:
    ...
    if current_state == AddJobtype.confirm:
        ...
        for category_id in selected_categories:
            ...
            try:
                ...
                await callback.message.edit_text(f"Ошибка при сохранении перевода: {e}",
                                                 reply_markup=keyboard.as_markup())
                return

        # save to DB
            try:
                await AsyncOrm.create_jobtype(new_key, category_id, session=session)
            except Exception as e:
    ...
    # FOR ADD
    if current_state == AddJobtype.confirm:
        text = f"✅ Группа узлов успешно создана\n"

        all_categories = await AsyncOrm.get_all_categories(session)
        for category in all_categories:
            if category.id in selected_categories:
                text += f"{category.emoji + ' ' if category.emoji else ''}{await t.t(category.title, lang)} -> \"{jobtype_title}\"\n"
